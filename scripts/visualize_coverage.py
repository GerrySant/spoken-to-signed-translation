#!/usr/bin/env python3
"""Visualize gloss coverage from a JSON file produced by text_to_gloss_to_pose_bulk --coverage-stats."""
...
def visualize(coverage_path: str):
    ...
    for i, sentence_tokens in enumerate(data["sentences"]):
        sentence_text = " ".join(t["word"] for t in sentence_tokens if t.get("word"))
        colored_glosses = " ".join(
            _colored(t["gloss"], t.get("coverage_type")) for t in sentence_tokens
        )
